...
interface TaskCardProps {
  ...
  finishTask: (title: string) => void
  unfinishTask: (title: string) => void
}
...
export function TaskCard({
  ...
  function handleCheckTask() {
    if (task.isFinished) {
      unfinishTask(task.title)
    } else {
      finishTask(task.title)
    }
  }
  ...
  return (
    <div className={styles.taskCard}>
      <div className={styles.taskTitle}>
        <Checkbox isChecked={task.isFinished} onClick={handleCheckTask} />
        <p
          style={{
            ...(task.isFinished && {
